...
long long int getPos(prhat_seqio *bs) {
       return gztell(bs->ks->f->f) - bs->ks->f->end + bs->ks->f->begin;
}

void setPos(prhat_seqio *bs, long long int pos) {
       bs->pos = pos;
       gzseek(bs->ks->f->f, bs->pos, SEEK_SET);
       ks_rewind(bs->ks->f);
}

long long int gzsize(const char *fn) {
    struct stat st;
...
void sharePositions (prhat_seqio *bs) {
       int rank, size;
...
void skipToNextRecord(prhat_seqio *bs) {
	   kstring_t buf;
       buf.l = buf.m = 0;
       buf.s = NULL;
       buf.rs = NULL;
...
void prhat_seq_close(prhat_seqio *bs)
{
	if (bs == 0) return;
...
uint32_t Aligner::gen_sed(char *bases, uint32_t len_bases, uint32_t *_sed_rec)
{
	...
	/*cout<<"pre= "<<pre<<endl;
	cout<<"hashtab->pointer= "<<hashtab->pointer<<endl;
	cout<<"hashtab->pointer[pre]= "<<hashtab->pointer[pre]<<endl;*/
			...
			if (_sam_details[i]) {
				...
				if (_sam_details[i]&1) {
					...
					for (int j=0; j< countSam; ++j) {
						...
						strcat(rank,rank_str);

						char endname[15]="_output.fastq";
						strcat(rank,endname);

						char fname[100];
						sprintf(fname,"%s",opt->hashdir);
						strcat(fname,"/");
						strcat(fname,rank);

						fp_out=fopen(fname,"a+");
						if(fp_out==NULL)
						{
						...
						//fprintf(fp_out,"%s\t%s\n",trunk->name.s,_usedread);
						//fprintf(fp_out,"%s,%s\n",trunk->name.s,nonSv[j].cigar.c_str());

						fprintf(fp_out,"%s\t%u\t%s\t%lu\t%d\t%s\t*\t0\t0\t%s\t%s\tAS:i:%d\n", trunk->name.s,nonSv[j].flag,ChrName[nonSv[j].chrIndex],nonSv[j].pos,nonSv[j].MAQ,nonSv[j].cigar.c_str(),_usedread,usedqual,nonSv[j].score);
					...
					for (int j=0; j < countSam; ++j) {
						...
						char fname[100]="/home/tr/Desktop/rHAT-master/rHAT_code/src4/";
						strcat(fname,rank);
						...
						//fprintf(fp_out,"%s\t%s\n",trunk->name.s,_usedread);
						//fprintf(fp_out,"%s,%s\n",trunk->name.s,sv[j].cigar.c_str());

						fprintf(fp_out,"%s\t%u\t%s\t%lu\t%d\t", trunk->name.s,sv[j].flag,ChrName[sv[j].chrIndex],sv[j].pos,sv[j].MAQ);
						if (sv[j].lclip)
						fprintf(fp_out,"%uS%s",sv[j].lclip,sv[j].cigar.c_str());
						if (sv[j].rclip)
						fprintf(fp_out,"%uS\t*\t0\t0\t%s\t%s\tAS:i:%d\n",sv[j].rclip,_usedread,usedqual,sv[j].score);
...
int Aligner::rhat_seq_read(kstream_t *_fp, kseq_t *_seqs, int n_needed, prhat_seqio *bs)
{
	kseq_t *temp = _seqs;
...
static void *thread_worker(void *data)
{
	...
	//read_tmp_num=ceil((double)read_num/comm_sz);

	//printf("###form my_rank=%d\t bs->read_rank_Pos=%d\t bs->read_rank_endPos=%d\n",my_rank,bs->read_rank_Pos,bs->read_rank_endPos);
	
	//cout<<"from my_rank= "<<my_rank<<" aux->n_seqs= "<<aux->n_seqs<<"  read_tmp_num= "<<read_tmp_num<<endl;
	int _read_seq;
	...
	while (1) {
		...
		//if(_read_seq<my_rank*read_tmp_num || _read_seq>=(my_rank+1)*read_tmp_num)
		//break;
		//}
		//cout<<"from my_rank= "<<my_rank<<" _read_seq= "<<_read_seq<<endl;
...
void Aligner::Runtask()
{
	...
	prhat_seqio *bs = NULL;
	bs = (prhat_seqio*)calloc(1, sizeof(prhat_seqio));
	...
	gzFile fp;
	fp = xzopen(opt->readpath, "r");
	bs->ks = kseq_init(fp);

	long long int fileSize = gzsize(opt->readpath);

	MPI_Bcast(&fileSize, 1, MPI_LONG_LONG, 0, MPI_COMM_WORLD);
	setPos(bs, my_rank * (fileSize/comm_sz));

	bs->endPos = (my_rank == (comm_sz-1)) ? gzsize(opt->readpath) : (my_rank+1) * (fileSize/comm_sz);

	if (my_rank != 0)
              skipToNextRecord(bs);
	
	//fprintf(stderr, "Proc %d: bs->pos= %lld  bs->endPos= %lld  fileSize= %lld\n", my_rank, bs->pos, bs->endPos,fileSize);

	sharePositions(bs);

	gzseek(fp, 0L, SEEK_SET);	
	
	/*uint32_t pointer_amount = 0;
	uint32_t seq_bkt_amount = 0;
	...
	pointer_amount = 1 << (opt->len_sed<<1)+1;
	seq_bkt_amount = len_genome-opt->len_sed + 1;

	//cout<<"pointer_amount= "<<pointer_amount<<endl;
	//cout<<"seq_bkt_amount= "<<seq_bkt_amount<<endl;
	...
	MPI_Bcast(&pointer_amount, 1, MPI_INT, 0, MPI_COMM_WORLD);
	MPI_Bcast(&seq_bkt_amount, 1, MPI_INT, 0, MPI_COMM_WORLD);

	if (my_rank != 0) {
        hashtab=(Hashtab*)calloc(1,sizeof(Hashtab));
        hashtab->pointer = (uint32_t*)calloc(pointer_amount, sizeof(uint32_t));
        hashtab->seq_bkt = (uint32_t*)calloc(seq_bkt_amount, sizeof(uint32_t));
		//cout<<"rank= "<<my_rank<<" pointer_amount= "<<pointer_amount<<endl;
	    //cout<<"rank= "<<my_rank<<" seq_bkt_amount= "<<seq_bkt_amount<<endl;
    }
//cout<<"sizeof MPI_UNSIGNED = "<<sizeof(MPI_UNSIGNED)<<endl;
	MPI_Bcast(hashtab->pointer, pointer_amount, MPI_UNSIGNED_SHORT, 0, MPI_COMM_WORLD);
    MPI_Bcast(hashtab->seq_bkt, seq_bkt_amount, MPI_UNSIGNED_SHORT, 0, MPI_COMM_WORLD);*/
	...
	if (opt->thread <= 1) {
		...
		int count = 0;
			...
			while (kseq_read(bs->ks)>=0) 
			{		
				if(getPos(bs)<=bs->endPos){

			revComRead(bs->ks->seq.s, bs->ks->seq.rs, bs->ks->seq.l);

			uint16_t sam_details = 0;

			sam_details = applyNonSV(bs->ks, rhashtab, rrhashtab, sams, sed_rec, sed_hit_times, unused_bkt);

			if (!sam_details && !(bs->ks->seq.l < LEN_BASES)) 
			{
				sam_details = applySV(bs->ks, rhashtab, rrhashtab, svsams, sed_rec, sed_hit_times, unused_bkt);
			...
			outputSam(bs->ks, sams, &svsams, &sam_details, 1);
			count++;
		    }
			}
			kseq_destroy(bs->ks);
		...
		while((n_seqs = rhat_seq_read(_fp, seqs, n_needed, bs))>0) {
			read_seq = 0;
			pthread_t *tid;
			//pthread_attr_t attr;
